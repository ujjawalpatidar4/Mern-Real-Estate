...
export default function CreateListing() {
  ...
  const handleImageUpload = async () => {
    ...
    if (files.length + cloudImageUrls.length > 6) return setError('You can only upload up to 6 images in total.');
    setUploading(true);
    setError('');
    const data = new FormData();
    files.forEach((file) => data.append('images', file));
    try {
      const res = await fetch('/api/listing/upload-cloudinary', {
        method: 'POST',
        body: data,
      });
      const result = await res.json();
      if (res.ok) {
        setCloudImageUrls(prev => [...prev, ...result.imageUrls].slice(0, 6));
        setFiles([]);
      } else {
  ...
  const handleRemoveImage = (index) => {
    setCloudImageUrls((prev) => prev.filter((_, i) => i !== index));
  };
    ...
    <main className='p-3 max-w-4xl mx-auto'>
      ...
      >
        ...
        <div className='flex flex-col flex-1 gap-4'>
          ...
          {cloudImageUrls.length > 0 &&
            cloudImageUrls.map((url, index) => (
              <div
                key={url}
